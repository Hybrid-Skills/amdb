            ...
            <div className="flex gap-6 flex-wrap">
              ...
              {data.revenue && data.revenue > 0 && (
                <div className="flex flex-col p-4 rounded-xl border border-green-500/20 bg-green-500/5 min-w-[140px]">
                  <span className="text-xs text-green-400/60 uppercase font-bold tracking-wider mb-1">
                    Revenue
                  </span>
        ...
        {(data.productionCompanies.length > 0 ||
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {data.productionCompanies.length > 0 && (
                <div>
                  <p className="text-xs text-white/40 uppercase font-bold tracking-wider mb-2">
                    Studios
                  </p>
                  <div className="flex flex-col gap-1">
                    {data.productionCompanies.map((c) => (
                      <p key={c.id} className="text-sm text-white/80">
                        {c.name}
                      </p>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-1">
                    {data.spokenLanguages.map((l) => (
                      <p key={l.iso_639_1} className="text-sm text-white/80">
                        {l.english_name}
                      </p>
                    ))}
                  </div>
              ...
              {data.networks && data.networks.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-col gap-1">
                    {data.networks.map((n) => (
                      <p key={n.id} className="text-sm text-white/80">
                        {n.name}
                      </p>
                    ))}
                  </div>
        ...
        {(data.externalIds.imdb_id ||
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {data.externalIds.instagram_id && (
                <a
                  href={`https://www.instagram.com/${data.externalIds.instagram_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
              ...
              {data.externalIds.twitter_id && (
                <a
                  href={`https://www.twitter.com/${data.externalIds.twitter_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
              ...
              {data.externalIds.facebook_id && (
                <a
                  href={`https://www.facebook.com/${data.externalIds.facebook_id}`}
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-2">
              {data.keywords.map((kw) => (
                <span
                  key={kw}
                  className="px-2.5 py-1 text-[11px] rounded-full bg-white/5 border border-white/10 text-white/50 hover:text-white/80 transition-colors font-medium"
                >
                  {kw}
                </span>
              ))}
