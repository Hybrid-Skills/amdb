  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-1 no-scrollbar">
      {profiles.map((p) => (
        ...
        >
          <div
            className={`rounded-full transition-all ${p.id === activeId ? 'ring-2 ring-white ring-offset-2 ring-offset-black' : ''}`}
          >
            <AvatarDisplay
              name={p.name}
          ...
          <span className="text-[11px] font-medium text-white/70 max-w-[56px] truncate">
            {p.name}
          </span>
        </button>
      ))}
